      ...
      itemWrap.innerHTML = `
        ...
        <div class="voucher-details-expand">
          ...
          <ul class="list-unstyled mb-0 px-2 extra-small">
            ...
            ${v.requiresUpsaleSlow ? `<li class="mb-1"><i class="bi bi-stars text-primary me-1"></i>Yêu cầu đơn có ít nhất một món bán chậm</li>` : ''}
            ...
            <li><i class="bi bi-clock text-warning me-1"></i>Hạn dùng: ${(v.expiresAtUtc || v.expiresAt) ? new Date(v.expiresAtUtc || v.expiresAt).toLocaleDateString('vi-VN') : 'Không giới hạn'}</li>
          </ul>
        </div>`;
